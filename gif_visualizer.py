...
To render Sound Reactive effects, you must set up the matrix and LED count to match the columns and rows here.
The max number of pixels is 180 see (MAX_LIVE_LEDS).
"""
...
def render_effect(fx=0, fp=None, ix=None, frame_cnt=None):
    ...
    if fp is None:
        # use fire palette for fire effects
        if fx in [66, 149, 151]:
            fp = 35
        else:
            fp = EFFECT_PALETTE
    ...
    if ix is None:
        ix = 127
    name = node.effects[fx]
    ai = name.find('@')
    if ai:
        name = name[0:ai]

    node.win(fx, fp, sx, ix)
    print(f'rendering effect {fx} {name}')
    time.sleep(0.7)
    frames = []
    ...
    frame_one.save(f'gifs/FX_{fx:03d}.gif', format="GIF", append_images=frames,
                   save_all=True, duration=40, loop=0, quality=10)
...
def render_all_effects(static_mode=False, min_fx=0, max_fx=AC_EFFECT_CNT, name_filter=None):
...
if __name__ == "__main__":
    ...
    parser.add_argument('--effect_filter', dest='effect_filter',
                        help='Only render effects where then name includes (i.e. 2D, ♪, ♫)')
